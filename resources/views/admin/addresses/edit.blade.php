    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.addresses.update", [$address->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('landmark') ? 'is-invalid' : '' }}" type="text" name="landmark" id="landmark" value="{{ old('landmark', $address->landmark) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('delivary') ? 'is-invalid' : '' }}" name="delivary_id" id="delivary_id" required>
                    @foreach($delivaries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('delivary_id') ? old('delivary_id') : $address->delivary->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
