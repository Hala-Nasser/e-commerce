    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.order-details.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_weight') ? 'is-invalid' : '' }}" type="number" name="total_weight" id="total_weight" value="{{ old('total_weight', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('invintory') ? 'is-invalid' : '' }}" name="invintory_id" id="invintory_id" required>
                    @foreach($invintories as $id => $entry)
                        <option value="{{ $id }}" {{ old('invintory_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
