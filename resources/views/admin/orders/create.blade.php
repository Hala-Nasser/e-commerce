    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_order_weight') ? 'is-invalid' : '' }}" type="number" name="total_order_weight" id="total_order_weight" value="{{ old('total_order_weight', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('delivery_time') ? 'is-invalid' : '' }}" type="text" name="delivery_time" id="delivery_time" value="{{ old('delivery_time') }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Order::WITH_DELIVERY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('with_delivery') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="with_delivery_{{ $key }}" name="with_delivery" value="{{ $key }}" {{ old('with_delivery', '') === (string) $key ? 'checked' : '' }} required>
